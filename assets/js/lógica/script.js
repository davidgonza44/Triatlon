...
function crear_tabla(id_div) {
    ...
    // Agregamos una clase personalizada al div contenedor
    table.classList.add('tabla-container');

    table.innerHTML = `
        <div class="content-wrapper">
            <h1 class="mb-4">Disciplina ${inicialMayuscula(id_div)} Resultados</h1>
            <table class="table table-info table-custom">
                <thead>
    ...
    `;
    const tablaElement = table.querySelector('table');
    tablaElement.classList.add('table-info');
...
.tabla-container {
    position: relative;
    background-image: url('lacava.jpg');
...
.table-info {
    --bs-table-bg: rgba(186, 230, 253, 0.8) !important;
    --bs-table-striped-bg: rgba(167, 207, 228, 0.8) !important;
    --bs-table-hover-bg: rgba(300, 197, 218, 0.9) !important;
}
...
/* Mantener el fondo oscuro del encabezado */
.table-dark {
...
/**
 * Cancela el cronómetro y reinicia sus variables a cero.
 * Detiene los intervalos Y limpia las tablas de resultados
 */
...
/**
 * Inicia el triatlón al validar que hay participantes seleccionados y configura
 * los elementos de la interfaz y las variables necesarias para comenzar la competencia.
 * Desactiva el botón de inicio, habilita los botones de reinicio y detener/reanudar,
 * y reinicia el cronómetro y las tablas de resultados. Crea una lista de participantes
 * seleccionados y los inicializa para la disciplina de "caminata". Comienza el cronómetro
 * y la actualización de la tabla por disciplina a intervalos regulares.
 */
function iniciar_triatlon() {
    ...
    const hIni = new Date(); // CREE ESTA NUEVA CONSTANTE
    caminata = seleccionados.map(participante => ({
        ...participante,
        varhi: hIni, // LA GUARDE EN UN OBEJETO NUEVO PARA QUE CADA PARTICPANTE TENGA LA HORA EN FORMATO NEW DATE
        hora_inicio: hIni.toLocaleTimeString(), // LA CONVERTI A STRING PARA MOSTRARLA
        hora_fin: "00:00:00",
        recorrido: 0,
        tiempo: "00:00:00",
        tiempo_seg_inicio: 0,
...
/**
 * Verifica si todos los participantes han finalizado o han sido descalificados en la disciplina actual.
 * @param {number} recorrido - Distancia para considerar que ha finalizado.
 * @param {Object[]} participantes - Arreglo de objetos con la información de los participantes.
 * @return {boolean} True si todos los participantes han finalizado o han sido descalificados, false en caso contrario.
 */
 ...
 * @return {number} Un valor aleatorio entre min y max.
 */
...
/**
 * Ordena el arreglo de participantes por su tiempo de finalización y recorrido.
 * Prioriza a los participantes que han sido descalificados.
 * @param {Object[]} participantes - Arreglo de objetos con la información de los participantes.
 * @return {Object[]} El arreglo ordenado.
 */
...
/**
 * Actualiza la información de los participantes en la competencia.
 * @param {Object[]} participantes - Arreglo de objetos con la información de los participantes.
...
function actualizarDatos(participantes) {
    participantes.forEach(
        participante => {
            ...
            const hFin = new Date(participante.varhi.getTime() + segundos*1000);//THIS ESTA


            participante.tiempo_seg_fin = segundos;
            participante.hora_fin = hFin.toLocaleTimeString()
...
/**
 * Función principal que se encarga de la lógica del triatlón.
 * Se encarga de:
 * 1. Actualizar la información de los participantes.
 * 2. Ordenarlos por su tiempo de finalización y recorrido.
 * 3. Actualizar la tabla de resultados.
 * 4. Si todos los participantes han finalizado o han sido descalificados, determinar el ganador y mostrar un mensaje emergente.
 * 5. Si la disciplina actual es caminata y todos los participantes han finalizado o han sido descalificados, cambiar la disciplina a natación.
 * 6. Si la disciplina actual es natación y todos los participantes han finalizado o han sido descalificados, cambiar la disciplina a ciclismo.
 * 7. Si la disciplina actual es ciclismo y todos los participantes han finalizado o han sido descalificados, mostrar un mensaje emergente con el ganador.
 * @return {undefined} No devuelve nada.
 */
function main() {
